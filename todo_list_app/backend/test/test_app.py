...
def test_delete_task(client):
    ...
    # タスクIDを取得
    # C
    deleted_task_id = Task.query.first().id
    
    # タスクを削除
    response = client.delete(f'/tasks/{deleted_task_id}')
    ...
    # 削除後タスクが存在しないことを確認
    with app.app_context():
        deleted_task = db.session.get(Task, deleted_task_id)
